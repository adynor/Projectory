...
$l_select_sql = 'select UR.UR_id, UR.UR_FirstName, UR.UR_MiddleName, UR.UR_LastName, UR.UR_Emailid, UR.UR_EmailidDomain,IT.IT_Name from Users as UR, Institutes as IT where UR.IT_id = IT.IT_id and UR.UR_Type = "A" and UR.UR_RegistrationStatus = "P"' ;
...
print ('<table class="ady-table-content" border=1 style="width:100%">');
        print ('<tr>'); 
        print ('<th>College Admin<br>Name</th>');
        print ('<th>College Admin<br>Email Id</th>');
        print ('<th>Institute<br>Name</th>');
        print ('<th>Select<br>College Admin</th>');
        
        print ('</tr>');
        if($l_UR_count == 0)
        {
            print ('<tr>');
            print('<td colspan=5>There are no new College Admins</td>');
        }
        else
        {
