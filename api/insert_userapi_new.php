<?php
 ...
 $l_insert_UR_pass = md5($l_UR_pass);
    ...
    {
        echo "Apologies!! Some error has occured while trying to run the program. Please try again.<a href='".$_SERVER['HTTP_REFERER']."'>Back</a>";
    }
    else
    {
        ...
        {
            $l_update_UR = 'update into Users set PR_id ='.$l_PR_id .' where UR_id = "'.$l_UR_id.'" AND Org_id="'.$l_Org_id.'"';
            mysql_query($l_update_UR);
            $URid_encode=md5(162).'=='.base64_encode($l_UR_id);
            $ORGid_encode=md5(162).'=='.base64_encode($l_Org_id);
            echo '<script>window.location.href="https://zaireprojects.com/test/loginOrganization.php?UR_id='.$URid_encode.'&ORG_id='.$ORGid_encode.'"</script>';
