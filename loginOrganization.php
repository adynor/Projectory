...
<<<<<<< HEAD
        $l_query_user_details=mysql_query('select UR.UR_id,UR.IT_id, UR.PG_id, UR.UR_Type,UR.TM_id,UR.UR_Semester, UR.PR_id,UR.UR_USN,UR.UR_PR_Type,UR.Org_id from  Users as UR where UR.UR_id="'.$OR_URid.'" AND UR.Org_id="'.$OR_Orgid.'" AND UR.UR_RegistrationStatus="C"');
        $l_row_users=mysql_fetch_array($l_query_user_details);
      
        $_SESSION['g_UR_id']=$l_row_users['UR_id'];
        $_SESSION['g_IT_id']=$l_row_users['IT_id'];
        $_SESSION['g_PG_id']=$l_row_users['PG_id'];
        $_SESSION['g_UR_Type']=$l_row_users['UR_Type'];
        $_SESSION['g_TM_id']=$l_row_users['TM_id'];
        $_SESSION['g_Semester_id']=$l_row_users['UR_Semester'];
        $_SESSION['g_PR_id']=$l_row_users['PR_id'];
        $_SESSION['g_UR_PR_Type']=$l_row_users['UR_PR_Type'];
        ...
        if($_SESSION['g_UR_Type']=='S' && $_SESSION['g_PR_id']!=NULL){
            ...
            echo '<h3 style="color:red">!!!Sorry Please Try Again<a href="#">Back Home</a> </h3>';
=======
$l_query_user_details=mysql_query('select UR.UR_id,UR.IT_id, UR.PG_id, UR.UR_Type,UR.TM_id,UR.UR_Semester, UR.PR_id,UR.UR_USN,UR.UR_PR_Type,UR.Org_id from  Users as UR where UR.UR_id="'.$OR_URid.'" AND UR.Org_id="'.$OR_Orgid.'" AND UR.UR_RegistrationStatus="C"');
        $l_row_users=mysql_fetch_array($l_query_user_details);
  $_SESSION['g_UR_id']=$l_row_users['UR_id'];
