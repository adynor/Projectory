...
//////////////////////filter starts
 ...
 $l_PR_Name = 'Dummyname';
        ...
        if ($l_PR_Name == 'Dummyname' && $l_SD_Name=='Dummyname' &&  $l_MO_Name =='Dummyname' )/////// filter
        {
         ...
         echo   $l_query_project='select  PR.PR_id, PR.PR_Name, PR.PR_Desc, PR.PR_SynopsisURL,PR.PR_Duration,MO.MO_Name,SO.SO_Name from Projects as PR,Model as MO,Project_Solution as PrS,Solution as SO, SubDomain as SD, Project_SubDomains as PS  where MO.MO_id=PR.MO_id and PR.PR_Status="C" and  PR.PR_ReleaseDate<='.$l_currentDate.' and PR.PR_ExpiryDate >='.$l_currentDate.' AND PrS.PR_id=PR.PR_id AND  AND PrS.SO_id=SO.So_id group by PR.PR_id order by PR.PR_Name ';
